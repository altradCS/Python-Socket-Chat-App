...
class client_ui():
    def __init__(self,root):
        ...
        self.disconnect_img = ctk.CTkImage(Image.open("assets/icons8-disconnect-50.png"), size=(30,30))
        ...
        self.close = ctk.CTkButton(self.title_frame,
                               ...
                               hover_color="red",
                               corner_radius=0,
                               command=lambda: close_win(root,self),
                               width=40)
    ...
    def add_client_device(self):
        ...
        ask_info.geometry("300x150")
        ask_info.attributes('-topmost', 'true')
        ctk.CTkLabel(ask_info, text="Enter: ", font=("JetBrains Mono", 12, "italic")).grid(row=0, column=0, sticky="w", padx=5)
        name = ctk.CTkEntry(ask_info, font=("JetBrains Mono", 12), placeholder_text="Username", width=280)
        name.grid(row=1, column =0, columnspan=2, pady=5, padx=10)
        email = ctk.CTkEntry(ask_info, font=("JetBrains Mono", 12), placeholder_text="Email Address", width=280)
        email.grid(row=2, column =0, columnspan=2, pady=5)
        submit = ctk.CTkButton(ask_info, text="Connect", font=("JetBrains Mono", 12), command=lambda: self.add_client(name.get(), email.get(), ask_info), width=100)
        submit.grid(row=3, column =0, pady=10, padx =2, columnspan=2)

    def add_client(self, name, email, ask_info):
        permission = True
        if server.clients:
            for conn in server.clients:
                if server.clients[conn]["username"] == name or server.clients[conn]["email"]==email:
                    messagebox.showerror("Error!", "Username or Email is active! Try to use different username or email.")
                    permission=False
        if permission:
            try:
                connection = connect()
                connection.send(f"{name},{email}".encode(FORMAT))
                ...
                messagebox.showerror("Error!", "Server is offline! Make sure the server is running.")
                ask_info.destroy()
